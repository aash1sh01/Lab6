...
{ 
       ...
       * 1 Traverse until all process gets completely executed.
         ...
         - Calculate waiting time for each over process. *wt[i]= Completion time - arrival_time-burst_time*
  ...
  while (over != n){
    ...
    for (int i = current_process+1; i <n ; i++){
      if (current_time >= plist[i].art){
        if (waiting_time[i]!=0 && waiting_time[i] < waiting_time[current_process]){
          current_process = i;
        }
      }
    }
